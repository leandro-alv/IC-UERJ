...
toolbox.register("map", futures.map) #Replace default map with futures.map
...
def evalSymbReg(individual, smpa, smpb, smpc):
    ...
    total_hits += (raif1 + raif2 + raif3)

    if raif1 > 0 and raif1 == raif2:
        if raif3 == 2*raif1:
            total_hits *= 10
        else:
            total_hits *= 2
...
smpa = random.sample([float(x) for x in range(2,20)], k=5)
smpb = random.sample([float(x) for x in range(2,20)], k=5)
smpc = random.sample([float(x) for x in range(2,20)], k=5)
...
def main():
    ...
    pop, log = algorithms.eaSimple(pop, toolbox, 0.5, 0.4, 40, stats=mstats,
